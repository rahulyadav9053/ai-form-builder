  ...
  config.forEach((element) => {
    ...
    switch (element.type.toLowerCase()) {
      ...
      default:
        ...
        console.warn(
          `Unsupported element type "${element.type}" in schema generation.`
        );
    }
...
export function FormRenderer({ formConfig, formId }: FormRendererProps) {
  ...
  const formSchema = React.useMemo(
    () => generateSchema(formConfig.elements),
    [formConfig.elements]
  );
  ...
  const renderFormElement = (element: FormElement) => {
    ...
    return (
      <FormField
        ...
        key={key}
        render={(
          { field } // field includes onChange, onBlur, value, name, ref
        ) => (
            ...
            <FormControl>
              {(() => {
                switch (element.type.toLowerCase()) {
                  ...
                  default:
                    console.warn(
                      `Unsupported form element type: ${element.type}`
                    );
                    return (
                      <Input
                        {...field}
                        disabled
                        placeholder={`Unsupported type: ${element.type}`}
                      />
                    );
                }
              })()}
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)}>
            <CardContent className="p-6 space-y-1 pt-6">
              {formConfig.elements
                .filter((el) => el.type.toLowerCase() !== "submit")
                .map(renderFormElement)}
            </CardContent>
            <CardFooter className="flex justify-end p-6 pt-4">
              <Button
                type="submit"
                disabled={isSubmitting || startTime === null}
              ...
              >
                {isSubmitting ? (
                  ...
                  <>
                    ...
                    {formConfig.elements.find(
                      (el) => el.type.toLowerCase() === "submit"
